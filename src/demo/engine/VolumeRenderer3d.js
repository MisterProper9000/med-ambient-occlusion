...
export default class VolumeRenderer3d {
  ...
  setAmbientTextureMode(isoThreshold) {
    ...
    this.volumeUpdater.ambientTexture.set(this.volTexture, isoThreshold, this.volume.m_dataArray, 
      this.vol1, 
      this.vol2, 
      this.vol3);
      ...
      for (let j = 0; j < yLen; j += 2) {
        for (let i = 0; i < xLen; i += 2) {
          shrinked[kk * yDim * xDim + jj * xDim + ii] = (array[k * yLen * xLen + j * xLen + i]
                                                    ...
                                                    + array[k * yLen * xLen + (j + 1) * xLen + i + 1]
  ...
  initWithVolume(volume, box, nonEmptyBoxMin, nonEmptyBoxMax, isRoiVolume, isFULL3D) {
    ...
    //volume.m_dataArray
    //volume.m_xDim,m_yDim,m_zDim
    console.log("start volumes processing at");
    console.log(Date.now());
    this.vol1 = this.arrayShrink(this.volume.m_dataArray, this.volume.m_xDim / 2, this.volume.m_yDim / 2, this.volume.m_zDim / 2);
    console.log("vol1 processed successfully!");
    console.log(Date.now());
    this.vol2 = this.arrayShrink(this.vol1, this.volume.m_xDim / 4, this.volume.m_yDim / 4, this.volume.m_zDim / 4);
    console.log("vol2 processed successfully!");
    console.log(Date.now());
